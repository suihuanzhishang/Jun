 ...
 * @param {Boolean} isAppend           可选，true=追加到末尾（不复制表头），
 *                                     false/undefined=清空重写（包括表头的全量复制），默认 false
 */
function fuzzyMatchCopy(
    ...
    // 读取第 1 行（表头行）
    let srcHeaderRange = srcSheet.Range("A1").Resize(1, srcMaxCol);
    let srcHeader = srcHeaderRange.Value2;

    // 当表头只有一列时，Value2 返回标量，需手动包装为二维数组
    if (!Array.isArray(srcHeader)) srcHeader = [[srcHeader]];
    ...
    let startRow; // 数据行写入起始行号

    if (isAppend) {
        // ── 追加模式 ──────────────────────────────────────────────────────────
        // 不写表头，直接找到 A 列最后一行，从其下一行开始追加数据
        let lastRow = resSheet.Cells(resSheet.Rows.Count, 1).End(xlUp).Row;
        startRow = lastRow + 1;

        // 若结果表完全为空（lastRow=1 且第 1 行也无数据），至少从第 2 行开始
        if (startRow < 2) startRow = 2;

    } else {
        // ── 覆盖模式（全量复制，含表头） ──────────────────────────────────────
        // 清除结果表全部已用内容（从第 1 行开始，包括旧表头）
        let resUsedLastRow = resSheet.UsedRange.Row + resSheet.UsedRange.Rows.Count - 1;
        if (resUsedLastRow >= 1) {
            resSheet.Range("1:" + resUsedLastRow).ClearContents();
        }

        // 将源表头写入第 1 行
        let resHeaderRange = resSheet.Range("A1").Resize(1, srcHeader[0].length);
        resHeaderRange.Value2 = srcHeader;
